(function () 
{
	...
	let url_data = "http://localhost:8080/moviescacdb/Peliculas";
	...
	window.postData = function(e)
	{
		...
		.catch(error => 
		{
			Swal.fire(
			{
				icon: "error",
				title: "eror",
				text: "Error al tratar de insertar los datos"
			});
			btn.removeAttribute('disabled');
		});
	...
	document.addEventListener("DOMContentLoaded", (event) => 
	{
		...
		fetch(url_data).then(f => f.json()).then(a => 
		{
			for(let i = 0; i < a.length; i++)
			{
				...
				var nombre = clone.querySelector(".ud-blog-title");
				var descrip = clone.querySelector(".ud-blog-desc");
				var btn = clone.querySelector(".ud-blog-date");
				
				img.src = a[i].portada;
				nombre.innerText = a[i].nombre;
				descrip.innerText = a[i].descrip;
	...
	window.deleteData = function(e)
	{
		Swal.fire(
		{
			title: "¿Desea eliminar la pelicula "+ e.nombre + "?",
